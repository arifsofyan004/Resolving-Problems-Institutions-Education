# -*- coding: utf-8 -*-
"""Submission_Akhir_Menyelesaikan_Permasalahan_Institusi_Pendidikan.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Lo3eyPiTnfV7AZ3juoC7VwS6j8ZaFZA4
...
"""Berdasarkan analisis data dropout debtors, mayoritas dari mereka memiliki orang tua dengan pekerjaan tidak terampil dan tunggakan biaya kuliah. Ini menyoroti masalah keuangan dan sosio-ekonomi yang mungkin memengaruhi keputusan mereka untuk keluar. Rekomendasi termasuk menyediakan bantuan keuangan tambahan, meningkatkan program beasiswa, dan mengevaluasi kursus-kursus dengan tingkat dropout tinggi untuk meningkatkan retensi siswa. Dengan mengatasi tantangan ini, institusi pendidikan dapat meningkatkan kesempatan kesuksesan bagi semua siswa.
...
Berdasarkan analisis, mayoritas debitur yang dropout memiliki ibu dan ayah dengan pekerjaan tidak terampil atau dalam bidang layanan pribadi dan administrasi, yang menunjukkan latar belakang ekonomi yang kurang stabil. Sebagian besar dari mereka belum melakukan pembayaran uang kuliah dan tidak menerima beasiswa, mengindikasikan bahwa masalah keuangan adalah faktor utama dalam dropout. Selain itu, sebagian besar mahasiswa yang dropout adalah lajang.
...
Mengganti nilai dalam kolom 'Application_mode' dengan label yang sesuai
"""
...
"""Distribusi Course menunjukkan bahwa kursus 9991 memiliki tingkat dropout yang paling tinggi, dengan 85 mahasiswa dari total 155 yang mendaftar mengalami dropout. Distribusi Displaced menunjukkan bahwa mahasiswa yang tidak terdampak memiliki jumlah dropout yang lebih tinggi dibandingkan dengan yang terdampak, dengan 340 mahasiswa tidak terdampak dan 95 mahasiswa terdampak mengalami dropout. Distribusi International menunjukkan bahwa mayoritas mahasiswa adalah non-internasional, dengan hanya 5 dari 435 mahasiswa internasional yang mengalami dropout. Distribusi Daytime_evening_attendance menunjukkan bahwa mahasiswa yang menghadiri kelas pada siang hari memiliki jumlah dropout yang lebih tinggi dibandingkan dengan yang menghadiri kelas pada malam hari, dengan 310 mahasiswa yang menghadiri kelas pada siang hari dan 125 mahasiswa yang menghadiri kelas pada malam hari mengalami dropout.
...
Berdasarkan distribusi status dropout pada mode aplikasi "Over 23 years old", terlihat bahwa tingkat dropout mencapai puncaknya dengan persentase sebesar 55.41%.

Terdapat beberapa faktor seperti status perkawinan di mana mahasiswa yang belum menikah memiliki risiko dropout yang lebih tinggi. Di sisi lain, penerimaan beasiswa terbukti efektif dalam mengurangi risiko dropout. Namun yang paling menonjol adalah kinerja akademik yang lebih buruk pada semester pertama dan kedua dibandingkan dengan mahasiswa yang tidak dropout dan Beberapa kursus dengan jumlah dropout terbanyak adalah Management (evening attendance) dan Agronomy.
...
- mengevaluasi dan meningkatkan kursus-kursus dengan tingkat dropout tinggi untuk meningkatkan retensi mahasiswa.
...
Mengubah nilai di kolom Application_order menjadi "Primary" dan "Secondary"
"""

df['Application_order'] = df['Application_order'].apply(lambda x: "Primary" if x == 1 else "Secondary")
df['Application_order'].value_counts()
...
"""### Mapping kolom-kolom sesuai dengan deskripsinya agar lebih mudah dipahami dan untuk tahap deployment agar tampilan Aplikasi nya lebih baik"""
...
"""Mapping untuk kolom Displaced, Debtor, Tuition_fees_up_to_date, Gender, Scholarship_holder dan International"""
...
"""menghapus kolom 'Status_Dropout', 'Status_Enrolled', dan 'Status_Graduate' karena sudah tidak digunakan lagi."""

df.drop(columns=['Status_Dropout', 'Status_Enrolled', 'Status_Graduate'], inplace=True)
...
"""Untuk data numerik, dilakukan imputasi nilai yang hilang dengan median dan penskalaan menggunakan StandardScaler. Sedangkan untuk data kategorikal, nilai yang hilang diimputasi dengan nilai yang paling sering muncul dan dilakukan one-hot encoding untuk mengubahnya menjadi representasi biner"""
...
categorical_cols = ['Application_mode', 'Application_order', 'Course', 'Mothers_qualification',
                    ...
                    'Displaced', 'Gender', 'Scholarship_holder']
...
"""Melatih tiga model klasifikasi: Regresi Logistik, Random Forest, dan Gradient Boosting, untuk menentukan algoritma terbaik."""
...
"""Evaluasi untuk tiga model klasifikasi: Regresi Logistik, Random Forest, dan Gradient Boosting."""

models = ['Logistic Regression', 'Random Forest', 'Gradient Boosting']
predictions = [y_pred_logreg, y_pred_rf, y_pred_gb]
...
"""Berdasarkan hasil evaluasi, model Gradient Boosting adalah pilihan terbaik. Model ini menunjukkan akurasi tertinggi sebesar 84.3% dibandingkan dengan Logistic Regression (79.3%) dan Random Forest (82.6%). Selain itu, Gradient Boosting memiliki f1-score yang lebih tinggi, menunjukkan keseimbangan yang baik antara presisi dan recall.

Oleh karena itu, Gradient Boosting adalah pilihan terbaik di antara ketiga model yang diuji. Kemudian akan dilakukan tuning parameter lebih lanjut untuk meningkatkan kinerja model Gradient Boosting.
"""
...
model_xgb = GridSearchCV(xgb_model, param_grid=param_space_xgb, cv=3, scoring='accuracy')
...
"""Save preprocesing dan model untuk membuat aplikasi prediksi dropout menggunakan streamlit"""

joblib.dump(preprocessor, 'preprocessor.pkl')
joblib.dump(model_xgb, 'model.pkl')

"""Percobaan memprediksi data baru menggunakan model dan preprocessor yang telah dilatih"""
...
pred_prob = model.predict_proba(new_data_processed)
pred_class = model.predict(new_data_processed)
